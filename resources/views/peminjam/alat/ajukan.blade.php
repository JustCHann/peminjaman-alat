...
<head>
    ...
    <title>Ajukan Peminjaman · Alatify</title>
...
<body class="bg-gray-50 antialiased flex min-h-screen">
    ...
    <aside class="w-72 bg-gradient-to-b from-indigo-800 to-indigo-900 text-white shadow-xl flex-shrink-0 hidden md:flex flex-col">
        ...
        <div class="flex-1 px-4 pt-6 pb-4 space-y-1 overflow-y-auto">
            ...
            <a href="peminjam/" class="nav-item flex items-center gap-4 px-4 py-3.5 rounded-xl text-indigo-50/90 hover:text-white">
    ...
    <main class="flex-1 flex flex-col min-h-screen bg-gray-50/80">
        ...
        <header class="bg-white/80 backdrop-blur-md border-b border-gray-200/60 px-8 py-4 sticky top-0 z-10 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-lg md:text-2xl font-semibold text-gray-800 tracking-tight">Ajukan Peminjaman</h1>
            </div>
        ...
        <div class="p-6 md:p-8 flex-1 fade-in">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Form Pengajuan Peminjaman</h2>
                    <p class="text-sm text-gray-500 mt-1">Isi form di bawah untuk mengajukan peminjaman alat</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <form action="{{ route('peminjam.alat.simpan', $alat->id) }}" method="POST" class="p-6 md:p-8">
                    @csrf

                    <!-- Informasi Alat -->
                    <div class="mb-8 p-5 bg-gradient-to-r from-indigo-50 to-white rounded-xl border border-indigo-100">
                        <div class="flex items-start gap-4">
                            <div class="h-14 w-14 bg-indigo-100 rounded-xl flex items-center justify-center">
                                <ion-icon name="hardware-chip-outline" class="text-2xl text-indigo-600"></ion-icon>
                            </div>
                            <div class="flex-1">
                                <h3 class="text-lg font-bold text-gray-800">{{ $alat->nama }}</h3>
                                <p class="text-sm text-gray-500 mt-0.5">
                                    Kategori: {{ $alat->kategori ? $alat->kategori->nama : 'Tanpa Kategori' }}
                                </p>
                                <div class="mt-2 flex items-center gap-3">
                                    <span class="inline-flex items-center gap-1 text-sm">
                                        <ion-icon name="cube-outline" class="text-indigo-500"></ion-icon>
                                        Stok tersedia: 
                                        <strong class="text-indigo-600">{{ $alat->jumlah }}</strong> unit
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="jumlah" id="jumlah" 
                                       class="w-full pl-10 pr-4 py-3 border border-gray-200 bg-gray-50/50 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-300/50 focus:border-indigo-300 transition" 
                                       min="1" max="{{ $alat->jumlah }}" value="{{ old('jumlah', 1) }}" required>
                            </div>
                            <p class="text-xs text-gray-500 mt-1.5 flex items-center gap-1">
                                <ion-icon name="information-circle-outline" class="text-gray-400"></ion-icon>
                                Maksimal peminjaman: {{ $alat->jumlah }} unit
                            </p>
                            @error('jumlah')
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="date" name="tanggal_kembali" id="tanggal_kembali" 
                                       class="w-full pl-10 pr-4 py-3 border border-gray-200 bg-gray-50/50 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-300/50 focus:border-indigo-300 transition" 
                                       value="{{ old('tanggal_kembali', \Carbon\Carbon::now()->addDays(7)->format('Y-m-d')) }}" 
                                       min="{{ \Carbon\Carbon::now()->format('Y-m-d') }}" required>
                            </div>
                            <p class="text-xs text-gray-500 mt-1.5 flex items-center gap-1">
                                <ion-icon name="information-circle-outline" class="text-gray-400"></ion-icon>
                                Pilih tanggal kapan alat akan dikembalikan (maksimal 7 hari)
                            </p>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="text-xs text-blue-700">
                                ...
                                <ul class="list-disc list-inside mt-1 space-y-0.5">
                                    <li>Pengajuan peminjaman akan diproses oleh admin</li>
                                    <li>Status peminjaman akan berubah menjadi "Menunggu" sampai disetujui admin</li>
                                    <li>Pastikan mengembalikan alat tepat waktu untuk menghindari denda</li>
                                    <li>Jika melebihi batas waktu, status akan berubah menjadi "Terlambat"</li>
                                </ul>
                            </div>
                        </div>
                    </div>
    ...
    <div id="mobileSidebar" class="md:hidden fixed inset-0 z-50 hidden transition-all">
        ...
        <div class="absolute left-0 top-0 h-full w-72 bg-gradient-to-b from-indigo-800 to-indigo-900 shadow-xl overflow-y-auto">
            ...
            <div class="px-4 pt-4 pb-20">
                ...
                <a href="{{ route('peminjam.peminjaman.index') }}" class="flex items-center gap-4 px-4 py-3 text-indigo-50/90 hover:bg-white/10 rounded-xl">Riwayat</a>
