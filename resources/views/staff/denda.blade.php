...
<head>
    ...
    <title>Kelola Denda · Alatify</title>
...
<body class="bg-gray-50 antialiased flex min-h-screen">
    ...
    <aside class="w-72 bg-gradient-to-b from-indigo-800 to-indigo-900 text-white shadow-xl flex-shrink-0 hidden md:flex flex-col">
        ...
        <div class="flex-1 px-4 pt-6 pb-4 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route('staff.peminjaman.index') }}" class="nav-item flex items-center gap-4 px-4 py-3.5 rounded-xl text-indigo-50/90 hover:text-white">
    ...
    <main class="flex-1 flex flex-col min-h-screen bg-gray-50/80">
        ...
        <header class="bg-white/80 backdrop-blur-md border-b border-gray-200/60 px-8 py-4 sticky top-0 z-10 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-lg md:text-2xl font-semibold text-gray-800 tracking-tight">Kelola Denda</h1>
            </div>
        ...
        <div class="p-6 md:p-8 flex-1 fade-in">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Manajemen Denda</h2>
                    <p class="text-sm text-gray-500 mt-1">Kelola pembayaran denda peminjaman alat</p>
                </div>
                <div class="flex gap-3">
                    <a href="{{ route('staff.dashboard') }}" class="inline-flex items-center gap-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 px-5 py-2.5 rounded-xl shadow-sm transition font-medium text-sm">
                        <ion-icon name="arrow-back-outline" class="text-lg"></ion-icon>
                        Kembali
                    </a>
                    <a href="{{ route('staff.denda.cetak') }}" class="inline-flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-5 py-2.5 rounded-xl shadow-sm transition font-medium text-sm">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between stat-card">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Total Denda</p>
                        <p class="text-3xl font-bold text-gray-800 mt-1">
                            Rp {{ number_format($data->sum('denda'), 0, ',', '.') }}
                        </p>
                        <span class="text-xs text-indigo-600 bg-indigo-50 px-2 py-1 rounded-full mt-2 inline-block">keseluruhan</span>
                    </div>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between stat-card">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Belum Bayar</p>
                        <p class="text-3xl font-bold text-yellow-600 mt-1">{{ $data->where('status_denda','belum_bayar')->count() }}</p>
                        <span class="text-xs text-yellow-600 bg-yellow-50 px-2 py-1 rounded-full mt-2 inline-block">menunggu</span>
                    </div>
                    <div class="h-12 w-12 bg-yellow-100 rounded-xl flex items-center justify-center text-yellow-600">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between stat-card">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Sudah Bayar</p>
                        <p class="text-3xl font-bold text-green-600 mt-1">{{ $data->where('status_denda','sudah_bayar')->count() }}</p>
                        <span class="text-xs text-green-600 bg-green-50 px-2 py-1 rounded-full mt-2 inline-block">lunas</span>
                    </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-5">
                    <h2 class="text-lg font-semibold text-gray-800 flex items-center gap-2">
                        <ion-icon name="document-text-outline" class="text-indigo-600 text-xl"></ion-icon>
                        Data Denda Peminjaman
                    </h2>
                    <span class="text-xs text-gray-400 bg-white px-4 py-2 rounded-full border">{{ $data->count() }} transaksi</span>
                </div>
                                    ...
                                    <tr class="hover:bg-gray-50/60 transition">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-3">
                                                <div class="h-8 w-8 bg-gradient-to-br from-indigo-100 to-indigo-50 rounded-full flex items-center justify-center text-indigo-700 font-semibold text-xs">
                                                    {{ substr($p->user->nama ?? 'U', 0, 2) }}
                                                </div>
                                                <div>
                                                    <span class="font-medium text-gray-800">{{ $p->user->nama }}</span>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-2">
                                                <ion-icon name="hardware-chip-outline" class="text-gray-400 text-sm"></ion-icon>
                                                <span class="text-gray-700">{{ $p->alat->nama }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            @else
                                                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-green-100 text-green-700">
                                                    <ion-icon name="checkmark-done-outline" class="text-sm"></ion-icon>
                                                    Lunas
                                                </span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($p->status_denda == 'belum_bayar')
                                                <form action="{{ route('staff.konfirmasiDenda', $p->id) }}" method="POST" class="inline" onsubmit="return confirm('Konfirmasi pembayaran denda ini?')">
                                                    @csrf
                                                    <button type="submit" class="inline-flex items-center gap-1 bg-green-50 hover:bg-green-100 text-green-700 px-3 py-1.5 rounded-lg transition text-xs font-medium border border-green-200/50">
                                        ...
                                        <td colspan="5" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center justify-center text-gray-400">
                                                <div class="h-20 w-20 bg-gray-50 rounded-full flex items-center justify-center mb-3">
                                                    <ion-icon name="cash-outline" class="text-4xl text-gray-300"></ion-icon>
                                                </div>
                                                <p class="font-medium text-gray-600">Tidak ada data denda</p>
                                                <p class="text-sm text-gray-400 mt-1">Semua denda sudah lunas</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
    ...
    <div id="mobileSidebar" class="md:hidden fixed inset-0 z-50 hidden transition-all">
        ...
        <div class="absolute left-0 top-0 h-full w-72 bg-gradient-to-b from-indigo-800 to-indigo-900 shadow-xl overflow-y-auto">
            ...
            <div class="px-4 pt-4 pb-20">
                ...
                <a href="#" class="flex items-center gap-4 px-4 py-3 bg-white/15 text-white font-medium rounded-xl">Denda</a>
                ...
                <div class="mt-6 pt-6 border-t border-white/10">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <div class="flex-1">
                            <p class="text-sm font-semibold text-white">{{ Auth::user()->name ?? 'Staff' }}</p>
