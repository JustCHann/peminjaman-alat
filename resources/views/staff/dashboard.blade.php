...
<head>
    ...
    <title>Dashboard Staff · Alatify</title>
    ...
    <style>
        ...
        .stat-card:hover { transform: translateY(-2px); box-shadow: 0 20px 25px -5px rgb(0 0 0 / 0.05), 0 8px 10px -6px rgb(0 0 0 / 0.02); }
        @keyframes fadeIn {
    ...
    <main class="flex-1 flex flex-col min-h-screen bg-gray-50/80">
        ...
        <header class="bg-white/80 backdrop-blur-md border-b border-gray-200/60 px-8 py-4 sticky top-0 z-10 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-lg md:text-2xl font-semibold text-gray-800 tracking-tight">Dashboard Staff</h1>
            </div>
        ...
        <div class="p-6 md:p-8 flex-1 fade-in">
            ...
            <!-- Welcome Section -->
            <div class="bg-gradient-to-r from-indigo-50 to-white rounded-2xl p-6 border border-indigo-100 mb-8">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-0.5">Anda login sebagai <span class="font-medium text-indigo-600">Staff</span></p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-5 mb-8">
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between stat-card">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-800 mt-1">{{ count($alat) }}</p>
                        <span class="text-xs text-indigo-600 bg-indigo-50 px-2 py-1 rounded-full mt-2 inline-block">tersedia</span>
                    </div>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between stat-card">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-800 mt-1">{{ $totalPeminjaman }}</p>
                        <span class="text-xs text-amber-600 bg-amber-50 px-2 py-1 rounded-full mt-2 inline-block">keseluruhan</span>
                    </div>
            ...
            <!-- Tombol ke Persetujuan -->
            <div class="mb-10">
                <a href="{{ route('staff.peminjaman.index') }}" 
                   class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl shadow-sm transition font-medium text-sm">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($alat as $a)
                                    @php
                                        $stokStatus = $a->jumlah > 5 ? 'Aman' : ($a->jumlah > 0 ? 'Hampir Habis' : 'Habis');
                                        $stokColor = $a->jumlah > 5 ? 'bg-green-100 text-green-700' : ($a->jumlah > 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700');
                                        $statusIcon = $a->jumlah > 5 ? 'checkmark-circle-outline' : ($a->jumlah > 0 ? 'alert-circle-outline' : 'close-circle-outline');
                                        $statusColor = $a->jumlah > 5 ? 'text-green-600' : ($a->jumlah > 0 ? 'text-yellow-600' : 'text-red-600');
                                    @endphp
                                    <tr class="hover:bg-gray-50/60 transition">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-3">
                                                <div class="h-8 w-8 bg-gradient-to-br from-indigo-100 to-indigo-50 rounded-lg flex items-center justify-center text-indigo-600">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium {{ $stokColor }}">
                                                <ion-icon name="cube-outline" class="text-sm mr-1"></ion-icon>
                                                {{ $a->jumlah }} unit
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
