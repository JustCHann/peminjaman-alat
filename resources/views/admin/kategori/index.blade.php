<x-app-layout>
    ...
    <div class="min-h-screen bg-gray-50/80 flex">
        ...
        <main class="flex-1 flex flex-col min-h-screen bg-gray-50/80">
            ...
            <header class="bg-white/80 backdrop-blur-md border-b border-gray-200/60 px-6 md:px-8 py-4 sticky top-0 z-10 flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-2 text-sm md:text-base">
                        ...
                        <span class="font-semibold text-gray-800">Kategori Alat</span>
                    </div>
                </div>
            ...
            <!-- Page content - CRUD KATEGORI -->
            <div class="p-6 md:p-8 flex-1">
                <!-- Header & tombol tambah -->
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Daftar Kategori</h2>
                        <p class="text-sm text-gray-500 mt-1">Kelola semua kategori alat yang tersedia</p>
                    </div>
                    <a href="{{ route('admin.kategori.create') }}" class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl shadow-sm transition font-medium text-sm self-start sm:self-center">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="px-6 py-4 border-b border-gray-100 flex flex-wrap items-center justify-between gap-3 bg-gray-50/30">
                        <div class="flex items-center gap-2 text-sm text-gray-600">
                            <ion-icon name="pricetags-outline" class="text-emerald-600"></ion-icon>
                            <span>Total <span class="font-semibold text-gray-900">{{ count($kategoris) }}</span> kategori</span>
                        </div>
                        <div class="text-xs text-gray-500">
                            Digunakan untuk pengelompokan alat
                        </div>
                    </div>
                                ...
                                <tr class="hover:bg-gray-50/60 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('admin.kategori.edit', $kategori->id) }}" class="inline-flex items-center gap-1 bg-amber-50 hover:bg-amber-100 text-amber-700 px-3 py-1.5 rounded-lg transition text-xs font-medium border border-amber-200/50">
                                                <ion-icon name="create-outline" class="text-sm"></ion-icon>
                                                Edit
                                            </a>
                                            
                                            <form action="{{ route('admin.kategori.destroy', $kategori->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus kategori ini? Kategori yang memiliki alat tidak dapat dihapus.')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="2" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center text-gray-400">
                                            <ion-icon name="pricetags-outline" class="text-5xl mb-3"></ion-icon>
                                            <p class="font-medium text-gray-600">Belum ada data kategori</p>
                                            <p class="text-sm mt-1">Silakan tambah kategori baru</p>
                                            <a href="{{ route('admin.kategori.create') }}" class="mt-4 bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm inline-flex items-center gap-1">
