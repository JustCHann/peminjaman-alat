<x-app-layout>
    ...
    <div class="min-h-screen bg-gray-50/80 flex">
        ...
        <!-- ===== MAIN CONTENT (CRUD PENGEMBALIAN) ===== -->
        <main class="flex-1 flex flex-col min-h-screen bg-gray-50/80">
            ...
            <!-- Page content - CRUD PENGEMBALIAN -->
            <div class="p-6 md:p-8 flex-1">
                <!-- Header & tombol tambah -->
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Daftar Pengembalian</h2>
                        <p class="text-sm text-gray-500 mt-1">Kelola semua data pengembalian alat</p>
                    </div>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            <thead class="bg-gray-50/80 border-b border-gray-100">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Peminjam</th>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-gray-50/60 transition">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            <div class="h-8 w-8 bg-gradient-to-br from-emerald-100 to-emerald-50 rounded-full flex items-center justify-center text-emerald-700 font-semibold text-xs">
                                                {{ substr($item->peminjaman->user->nama ?? 'U', 0, 2) }}
                                            </div>
                                            <div>
                                                <span class="font-medium text-gray-800">{{ $item->peminjaman->user->nama ?? 'User Terhapus' }}</span>
                                                @if($item->peminjaman->user)
                                                <p class="text-xs text-gray-400">{{ $item->peminjaman->user->username ?? '' }}</p>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            <div class="h-7 w-7 bg-gray-100 rounded-lg flex items-center justify-center text-gray-600">
                                                <ion-icon name="construct-outline" class="text-sm"></ion-icon>
                                            </div>
                                            <div>
                                                <span class="font-medium text-gray-800">{{ $item->peminjaman->alat->nama ?? 'Alat Terhapus' }}</span>
                                                @if($item->peminjaman->alat)
                                                <p class="text-xs text-gray-400">{{ $item->peminjaman->alat->kode ?? '' }}</p>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-1.5">
                                            <div class="h-7 w-7 bg-emerald-50 rounded-lg flex items-center justify-center text-emerald-600">
                                                <ion-icon name="calendar-outline" class="text-sm"></ion-icon>
                                            </div>
                                            <span class="text-gray-700 font-medium">{{ \Carbon\Carbon::parse($item->tgl_pengembalian)->format('d M Y') }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            <form action="{{ route('admin.pengembalian.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus data pengembalian ini?')">
                                                @csrf
                                                @method('DELETE')
                                    ...
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center text-gray-400">
                                            <div class="h-20 w-20 bg-gray-50 rounded-full flex items-center justify-center mb-3">
                                                <ion-icon name="arrow-undo-outline" class="text-4xl text-gray-300"></ion-icon>
                                            </div>
                                            <p class="font-medium text-gray-600">Belum ada data pengembalian</p>
                                            <p class="text-sm text-gray-400 mt-1">Silakan tambah pengembalian baru</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
