<x-app-layout>
    ...
    <div class="min-h-screen bg-gray-50/80 flex">
        ...
        <aside class="w-72 bg-gradient-to-b from-indigo-800 to-indigo-900 text-white shadow-xl flex-shrink-0 hidden md:flex flex-col">
            ...
            <!-- Navigation - dengan active state di PEMINJAMAN -->
            <div class="flex-1 px-4 pt-6 pb-4 space-y-1 overflow-y-auto">
                ...
                <!-- PEMINJAMAN - ACTIVE -->
                <a href="{{ route('admin.peminjaman.index') }}" class="nav-item nav-active flex items-center gap-4 px-4 py-3.5 rounded-xl text-white font-medium bg-white/15 border-l-4 border-white pl-[14px]">
        ...
        <main class="flex-1 flex flex-col min-h-screen bg-gray-50/80">
            ...
            <!-- Page content - CRUD PEMINJAMAN -->
            <div class="p-6 md:p-8 flex-1">
                <!-- Header & tombol tambah -->
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Daftar Peminjaman</h2>
                        <p class="text-sm text-gray-500 mt-1">Kelola semua data peminjaman alat</p>
                    </div>
                        ...
                        <table class="w-full text-sm">
                            <thead class="bg-gray-50/80 border-b border-gray-100">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tgl Pinjam</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tgl Kembali</th>
                                ...
                                <tr class="hover:bg-gray-50/60 transition">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="font-medium text-gray-800">{{ $p->user->nama ?? 'User Terhapus' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            <span class="font-medium text-gray-800">{{ $p->alat->nama ?? 'Alat Terhapus' }}</span>
                                            @if($p->alat)
                                                <span class="text-xs text-gray-400">({{ $p->alat->kode ?? '' }})</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-1.5">
                                            <ion-icon name="calendar-clear-outline" class="text-gray-400 text-sm"></ion-icon>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span class="text-gray-600">{{ $p->tanggal_kembali ? \Carbon\Carbon::parse($p->tanggal_kembali)->format('d M Y') : '-' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">                        
                                            <form action="{{ route('admin.peminjaman.destroy', $p->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus data peminjaman ini?')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center text-gray-400">
                                            <ion-icon name="calendar-outline" class="text-5xl mb-3"></ion-icon>
                                            <p class="font-medium text-gray-600">Belum ada data peminjaman</p>
                                            <p class="text-sm mt-1">Silakan tambah peminjaman baru</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
    ...
    <div id="mobileSidebar" class="md:hidden fixed inset-0 z-50 hidden transition-opacity">
        ...
        <div class="absolute left-0 top-0 h-full w-72 bg-gradient-to-b from-indigo-800 to-indigo-900 shadow-xl">
            ...
            <div class="px-4 pt-4">
                ...
                <a href="{{ route('admin.peminjaman.index') }}" class="flex items-center gap-4 px-4 py-3 bg-white/15 text-white font-medium rounded-xl">Peminjaman</a>
